...
import { announceSchema, ErrorCode, SocketEvent, type AnnouncePlatform } from '@messenger/shared';
...
appVersionRouter.post('/announce', validateBody(announceSchema), (req, res, next) => {
  ...
  const platforms = (req.body as { platforms: AnnouncePlatform[] }).platforms;

  void (async () => {
    const android = platforms.includes('android') ? await appReleaseService.getAndroidRelease() : null;
    const windows = platforms.includes('windows') ? await appReleaseService.getWindowsRelease() : null;

    if (platforms.includes('android') && !android) {
      res.status(404).json({ error: { code: ErrorCode.NOT_FOUND, message: 'Выпуск Android не опубликован' } });
      return;
    }
    if (platforms.includes('windows') && !windows) {
      res.status(404).json({ error: { code: ErrorCode.NOT_FOUND, message: 'Выпуск Windows не опубликован' } });
      return;
    }

    const changelog = android?.changelog ?? windows?.changelog ?? [];
    if (android && windows && android.changelog.join('|') !== windows.changelog.join('|')) {
      res.status(409).json({
        error: {
          code: ErrorCode.VALIDATION_FAILED,
          message: 'Список изменений в android.json и windows.json различается — патчноут должен быть один',
        },
      });
      return;
    }
    ...
    const result = await announcementService.publishAnnouncement({
      androidVersionCode: android?.latestVersionCode ?? null,
      androidVersionName: android?.versionName ?? null,
      windowsVersionName: windows?.versionName ?? null,
      changelog,
    });
    ...
    res.json({
      androidVersionName: result.androidVersionName,
      windowsVersionName: result.windowsVersionName,
