...
import { readStoredFile } from '../services/file.js';
...
async function main(): Promise<void> {
  ...
  const total = await prisma.attachment.count({
    where: { blurhash: null, thumbnailFileId: { not: null } },
  });

  if (total === 0) {
    console.log('Вложений без blurhash не найдено.');
    return;
  }

  console.log(`${prefix}вложений без blurhash: ${total}`);
  ...
  for (;;) {
    const batch = await prisma.attachment.findMany({
      where: { blurhash: null, thumbnailFileId: { not: null }, id: { gt: cursor } },
      orderBy: { id: 'asc' },
      take: BATCH,
    ...
    const updates: { id: string; blurhash: string }[] = [];
    for (const attachment of batch) {
      const bytes = attachment.thumbnailFileId ? await readStoredFile(attachment.thumbnailFileId) : null;
      const blurhash = bytes ? await computeBlurhash(bytes) : null;
      if (blurhash) {
        updates.push({ id: attachment.id, blurhash });
      } else {
        skipped += 1;
    ...
    if (!dryRun && updates.length > 0) {
      await prisma.$transaction(
        updates.map(({ id, blurhash }) => prisma.attachment.update({ where: { id }, data: { blurhash } })),
      );
    }
