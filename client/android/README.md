...
другого — каталог `APP_RELEASE_DIR` (по умолчанию `releases/` в корне репозитория) с
...
4. Скопировать `releases/android.json` и `releases/qwill-<версия>.apk` в `releases/` рядом с
   развёрнутым сервером (`/var/www/message/releases/`). Перезапуск сервера не нужен: манифест
