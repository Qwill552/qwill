  ...
  useEffect(() => {
    ...
    const schedule = () => {
      ...
      timer = window.setTimeout(() => {
        const room = overlay.scrollHeight - overlay.clientHeight;
        setHint(room > HINT_MIN_ROOM && overlay.scrollTop < room * HINT_MAX_PROGRESS);
      }, HINT_IDLE_MS);
    };
    const onActivity = () => {
      setHint(false);
    ...
    ACTIVITY_EVENTS.forEach((name) => overlay.addEventListener(name, onActivity, { passive: true }));
    schedule();

    return () => {
      window.clearTimeout(timer);
      ACTIVITY_EVENTS.forEach((name) => overlay.removeEventListener(name, onActivity));
    };
  }, []);
