...
машине `80/443` уже держит системный Caddy — он же обслуживает `qwill.mooo.com` и другие
домены, в том числе сторонние, к Qwill не относящиеся. Второй процесс не может занять те
же порты.
...
- Открытые порты (см. `calls/01-livekit-infra.md`, раздел «Требования к машине»):
  ...
  Дополнительно: `30000-40000/udp` — это **не** тот же диапазон, что `rtc.port_range`
  (50000-60000, используется для ICE напрямую). Это отдельный диапазон TURN-relay
  (`turn.relay_range_start`/`turn.relay_range_end`), которого нет в дефолтном
  `livekit.yaml` этого шага и не было в исходном ТЗ. LiveKit слушает его по умолчанию
  без явной настройки в конфиге — обнаружено по логам контейнера
  (`service/turn.go: Starting TURN server {"turn.relay_range_start": 30000,
  "turn.relay_range_end": 40000, ...}`). Без него TURN-релей может не отдавать
  `relay`-кандидаты даже при полностью верной настройке остального.
