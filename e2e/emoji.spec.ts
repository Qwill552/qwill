...
async function registerUserForEmojiTest(
  page: Page,
  user: TestUser,
): Promise<void> {
...
test("эмодзи в отправленном сообщении рендерится спрайтом у обоих собеседников", async ({
  browser,
}) => {
  ...
  try {
    ...
    const sentRowOnA = pageA
      .locator(".message-wrap", { hasText: "привет" })
      .last();
...
test("эмодзи в превью списка чатов и в плашке ответа рендерится спрайтом", async ({
  browser,
}) => {
  const contextA = await browser.newContext();
  const contextB = await browser.newContext();
  ...
  try {
    ...
    const chatRowOnB = pageB
      .locator("nav")
      .locator("a", { hasText: userA.displayName });
    await expect(chatRowOnB.locator('span[style*="sheet.webp"]')).toBeVisible({
      timeout: 10_000,
    });
